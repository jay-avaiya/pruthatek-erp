...
const CardLeft = ({ title, description }) => {
  return (
    <div className="w-full h-fit px-4 md:px-6 lg:px-12 xl:px-24 flex flex-col lg:flex-row items-center justify-between font-inter gap-y-8 lg:gap-y-0 text-center lg:text-start">
      <div className="w-full hidden lg:block lg:max-w-[50%] xl:max-w-[45%] space-y-6">
        <p className="text-[16px] font-semibold text-[#F19623]">
          All Details in One Place
        </p>
        <h3 className="lg:text-3xl xl:text-[40px] text-[#282828] font-semibold leading-[45px]">
          {title}
        </h3>
        ...
        <button className="font-bold text-[16px] text-white bg-[#F19623] py-3 px-12 rounded-xl cursor-pointer active:scale-95 transition duration-100">
          Let's Start Now
        </button>
      </div>
      ...
      {/* mobile + tab */}
      <div className="w-full lg:hidden md:max-w-[80%] space-y-3">
        <p className="text-sm md:text-[16px] font-semibold text-[#F19623]">
          All Details in One Place
        </p>
        <h3 className="text-2xl md:text-3xl text-[#282828] font-semibold leading-[45px]">
          {title}
        </h3>
